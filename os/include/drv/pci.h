 ...
 * 文件:    pci.h
 *
 * 说明:    PCI总线相关定义(Peripheral Component Interconnect)
 *
 * 作者:    Jun
 *
 * 时间:    2010-7-12
 *
 * 声明:    部分内容来自ReactOS-0.2.7
************************************************************************************************/
#ifndef  FAMES_PCI_H
#define  FAMES_PCI_H
 ...
 *                   PCI配置空间
 * 
 * PCI_CFG_IDX_PORT:  索引端口
 * PCI_CFG_DAT_PORT:  数据端口
 * PCI_CFG_RW_ENABLE: PCI配置空间读写允许
 * PCI_MAX_BUS_NUM:   PCI总线数
 * PCI_MAX_DEV_NUM:   一条PCI总线上的最大设备数
 * PCI_MAX_FUNC_NUM:  一个PCI设备的最大功能数
 *
 * GET_PCI_CFG_ADDR:  由总线号,设备号,功能号计算某配置空间的地址
**--------------------------------------------------------------------------------------------*/
#define  PCI_CFG_IDX_PORT                0xCF8
#define  PCI_CFG_DAT_PORT                0xCFC

#define  PCI_CFG_RW_ENABLE               0x80000000UL

#define  PCI_MAX_BUS_NUM                 256
#define  PCI_MAX_DEV_NUM                 32
#define  PCI_MAX_FUNC_NUM                8

#define  GET_PCI_CFG_ADDR(bus,dev,func) ((PCI_CFG_RW_ENABLE)+((INT32U)(bus)*0x10000UL)+(((INT32U)(dev)*0x8UL+(INT32U)(func))*0x100UL))
#define  GET_PCI_CFG_ADDR2(bus,devfn)   ((PCI_CFG_RW_ENABLE)+((INT32U)(bus)*0x10000UL)+(((INT32U)(devfn))*0x100UL))


/*----------------------------------------------------------------------------------------------
 *                    PCI配置空间
 * 
 * 在PCI总线上, 每个设备均具有256个字节的配置空间, 其中前64个字节是标准化的, 如下所列:
**--------------------------------------------------------------------------------------------*/
 ...
 *                    PCI之无效ID号
**--------------------------------------------------------------------------------------------*/
#define  PCI_ID_INVALID       0xFFFFFFFFL
...
#define  PCI_CLASS_ETHERNET   0x0200         /* 以太网控制器          */         

/*----------------------------------------------------------------------------------------------
 *    函数声明
**--------------------------------------------------------------------------------------------*/
BOOL   pci_find_device_id   (INT16S venid, INT16S devid, INT16S * bus, INT16S * devfn, INT16S index);
